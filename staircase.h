...
void stairCase(Cube& cube, Shader& lightingShader, glm::mat4 als, float stairCount)
{
	//////////////////////////////////sphere and cylinder
	...
	for (float i = 0.0; i <= stairCount; i = i + 1.0)
	{
		//siri
		...
		cube.setMaterialisticEimu(0.239f, 0.157f, 0.051f);
	...
	for (float j = 0.0; j < stairCount; j++)
	{
		temp = glm::mat4(1.0f);
		temp = als;
	...
	scaleMatrix = glm::scale(identityMatrix, glm::vec3((7.0 / 12.0) * stairCount, .5, .25));
	...
	if(stairCount!=5.0f)
	{
		...
		temp = als;
		translateMatrix = glm::translate(identityMatrix, glm::vec3(-0.15, 1.75, 0.125));
		als = als * translateMatrix;
		sphere2.drawSphere(lightingShader, als, 0.6, 0.3f, 0.125f);
		als = temp;
		temp = als;
		translateMatrix = glm::translate(identityMatrix, glm::vec3(-0.15, 1.75, 2.925));
		als = als * translateMatrix;
		sphere2.drawSphere(lightingShader, als, 0.6, 0.3f, 0.125f);
		als = temp;
	}
...
void stairCaseCastle(Cube& cube, Shader& lightingShader, glm::mat4 als, float stairCount)
{
	//////////////////////////////////sphere and cylinder
	Sphere sphere = Sphere();
	Cylinder cylinder2 = Cylinder(.1, 1.6);
	...
	for (float i = 0.0; i <= stairCount; i = i + 1.0)
	{
		...
		scaleMatrix = glm::scale(identityMatrix, glm::vec3(.5, .25, 3.0));
		model = als * ttranslate * scaleMatrix;
		cube.setMaterialisticEimu(0.231f, 0.173f, 0.18f);
		cube.drawCubeWithMaterialisticProperty(lightingShader, model);
		//drawCube(cubeVAO, lightingShader, model  , 0.6, 0.3f, 0.125f);
	...
	for (float j = 0.0; j < stairCount; j++)
	{
		...
		als = als * translateMatrix;
		ttranslate = glm::translate(identityMatrix, glm::vec3((static_cast<float>(j) * .5) + .125, (static_cast<float>(j) * .25) + .25, 0.0));
		cylinder2.drawCylinder(lightingShader, als * ttranslate, 0.231f, 0.173f, 0.18f);
		//cylinder2.drawCylinder(lightingShader, als * ttranslate * (glm::rotate(identityMatrix, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f))), 0.6, 0.3f, 0.125f);
		als = temp;
	...
	if (stairCount != 5.0f)
	{
		...
		translateMatrix = glm::translate(identityMatrix, glm::vec3(-0.15, 1.75, 0.125));
		als = als * translateMatrix;
		sphere2.drawSphere(lightingShader, als, 0.231f, 0.173f, 0.18f);
		als = temp;
		temp = als;
		translateMatrix = glm::translate(identityMatrix, glm::vec3(-0.15, 1.75, 2.925));
		als = als * translateMatrix;
...
void stairCaseforClassroom(Cube& cube, Shader& lightingShader, glm::mat4 als)
{
	//////////////////////////////////sphere and cylinder
	Sphere sphere = Sphere();
	Cylinder cylinder2 = Cylinder(.1, 1.6);
	...
	temp = als;
	translateMatrix = glm::translate(identityMatrix, glm::vec3(0.0, 1.5, 0.0));
	scaleMatrix = glm::scale(identityMatrix, glm::vec3(5.5, .5, .25));
	rotateZMatrix = glm::rotate(identityMatrix, glm::radians(rotateAngle_Z), glm::vec3(0.0f, 0.0f, 1.0f));
	als = als * translateMatrix * rotateZMatrix * scaleMatrix;
	cube.setMaterialisticEimu(0.6, 0.3f, 0.125f);
	cube.drawCubeWithMaterialisticProperty(lightingShader, als);
